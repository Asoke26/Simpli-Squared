...
JOIN (SELECT ci.movie_id AS ci_movie_id FROM cast_info AS ci
 join role_type AS rt on (ci.note LIKE '%(producer)%' AND rt.role_t = 'actor' AND rt.id = ci.role_id ) join char_name AS chn on (chn.id = ci.person_role_id ))S1 on (t.id = S1.ci_movie_id AND S1.ci_movie_id = mc.movie_id );