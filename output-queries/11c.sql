...
JOIN (SELECT mc.movie_id AS mc_movie_id FROM movie_companies AS mc
 join company_type AS ct on (mc.note IS NOT NULL AND ct.kind != 'production companies' AND ct.kind IS NOT NULL AND mc.company_type_id = ct.id ) join company_name AS cn on (cn.country_code !='[pl]' AND (cn.name LIKE '20th Century Fox%'  OR cn.name LIKE 'Twentieth Century Fox%') AND mc.company_id = cn.id ))S1 on (t.id = S1.mc_movie_id AND ml.movie_id = S1.mc_movie_id ) 
