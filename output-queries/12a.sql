...
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info AS mi
 join info_type AS it1 on (mi.info IN ('Drama',  'Horror') AND it1.info = 'genres' AND mi.info_type_id = it1.id ))S2 on (t.id = S2.mi_movie_id AND S2.mi_movie_id = mi_idx.movie_id AND S1.mc_movie_id = S2.mi_movie_id );