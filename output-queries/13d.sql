...
JOIN (SELECT mc.movie_id AS mc_movie_id FROM movie_companies AS mc
 join company_type AS ct on (ct.kind ='production companies' AND ct.id = mc.company_type_id ) join company_name AS cn on (cn.country_code ='[us]' AND cn.id = mc.company_id ))S1 on (S1.mc_movie_id = t.id AND miidx.movie_id = S1.mc_movie_id ) 
