...
JOIN (SELECT mc.movie_id AS mc_movie_id FROM movie_companies AS mc
 join company_type AS ct on (mc.note LIKE '%(200%)%' AND mc.note LIKE '%(worldwide)%' AND ct.id = mc.company_type_id ) join company_name AS cn on (cn.country_code = '[us]' AND cn.id = mc.company_id ))S1 on (t.id = S1.mc_movie_id AND S1.mc_movie_id = att.movie_id ) 
