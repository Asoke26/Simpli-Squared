...
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info as mi join info_type as it1 on (mi.note LIKE '%internet%' AND it1.info = 'release dates' AND it1.id = mi.info_type_id)) S3 on (t.id = S3.mi_movie_id AND S3.mi_movie_id = att.movie_id AND S3.mi_movie_id = S1.mc_movie_id AND S2.mk_movie_id = S3.mi_movie_id );