...
JOIN (SELECT mc.movie_id AS mc_movie_id FROM movie_companies AS mc
 join company_name AS cn on (mc.note IS NOT NULL AND (mc.note LIKE '%(USA)%' OR mc.note LIKE '%(worldwide)%') AND cn.country_code ='[us]' AND cn.id = mc.company_id ))S2 on (S2.mc_movie_id = S1.ci_movie_id AND S1.t_id = S2.mc_movie_id ) 
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info AS mi
 join info_type AS it on (mi.info IS NOT NULL AND (mi.info LIKE 'Japan:%200%' OR mi.info LIKE 'USA:%200%') AND it.info = 'release dates' AND it.id = mi.info_type_id ))S3 on (S3.mi_movie_id = S1.ci_movie_id AND S1.t_id = S3.mi_movie_id AND S2.mc_movie_id = S3.mi_movie_id );