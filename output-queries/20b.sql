...
JOIN (SELECT ci.movie_id AS ci_movie_id FROM cast_info AS ci
 join char_name AS chn on (chn.name NOT LIKE '%Sherlock%' AND (chn.name LIKE '%Tony%Stark%' OR chn.name LIKE '%Iron%Man%') AND chn.id = ci.person_role_id ) join name AS n on (n.name LIKE '%Downey%Robert%' AND n.id = ci.person_id ))S2 on (t.id = S2.ci_movie_id AND S2.ci_movie_id = cc.movie_id AND S1.mk_movie_id = S2.ci_movie_id );