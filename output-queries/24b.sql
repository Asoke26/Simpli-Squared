...
JOIN (SELECT mc.movie_id AS mc_movie_id FROM movie_companies as mc join company_name as cn on (cn.country_code ='[us]' AND cn.name = 'DreamWorks Animation' AND cn.id = mc.company_id)) S2 on (S2.mc_movie_id = S1.ci_movie_id ) 
join title AS t on (t.production_year > 2010 AND t.title LIKE 'Kung Fu Panda%' AND t.id = S1.ci_movie_id AND t.id = S2.mc_movie_id ) 
JOIN (SELECT mk.movie_id AS mk_movie_id FROM movie_keyword as mk join keyword as k on (k.keyword IN ('hero',  'martial-arts', 'hand-to-hand-combat',  'computer-animated-movie') AND k.id = mk.keyword_id)) S3 on (t.id = S3.mk_movie_id AND S1.ci_movie_id = S3.mk_movie_id AND S2.mc_movie_id = S3.mk_movie_id ) 
