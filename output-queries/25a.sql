...
JOIN (SELECT mk.movie_id AS mk_movie_id FROM movie_keyword AS mk
 join keyword AS k on (k.keyword IN ('murder', 'blood',   'gore',   'death',  'female-nudity') AND k.id = mk.keyword_id ))S1 on (t.id = S1.mk_movie_id AND mi_idx.movie_id = S1.mk_movie_id ) 
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info AS mi
 join info_type AS it1 on (mi.info = 'Horror' AND it1.info = 'genres' AND it1.id = mi.info_type_id ))S2 on (t.id = S2.mi_movie_id AND S2.mi_movie_id = mi_idx.movie_id AND S2.mi_movie_id = S1.mk_movie_id ) 
