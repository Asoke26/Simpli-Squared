...
JOIN (SELECT mi_idx.movie_id AS mi_idx_movie_id FROM movie_info_idx AS mi_idx
 join info_type AS it2 on (mi_idx.info > '7.0' AND it2.info = 'rating' AND it2.id = mi_idx.info_type_id ))S1 on (t.id = S1.mi_idx_movie_id AND cc.movie_id = S1.mi_idx_movie_id ) 
JOIN (SELECT mk.movie_id AS mk_movie_id FROM movie_keyword AS mk
 join keyword AS k on (k.keyword IN ('superhero', 'marvel-comics', 'based-on-comic', 'tv-special', 'fight', 'violence', 'magnet', 'web', 'claw', 'laser') AND k.id = mk.keyword_id ))S2 on (t.id = S2.mk_movie_id AND S2.mk_movie_id = cc.movie_id AND S2.mk_movie_id = S1.mi_idx_movie_id ) 
