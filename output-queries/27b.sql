...
JOIN (SELECT cc.movie_id AS cc_movie_id FROM complete_cast AS cc
 join comp_cast_type AS cct1 on (cct1.kind IN ('cast',   'crew') AND cct1.id = cc.subject_id ) join comp_cast_type AS cct2 on (cct2.kind = 'complete' AND cct2.id = cc.status_id ))S1 on (t.id = S1.cc_movie_id AND ml.movie_id = S1.cc_movie_id ) 
