...
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info as mi join info_type as it on (mi.info LIKE 'USA:%200%' AND it.info = 'release dates' AND it.id = mi.info_type_id)) S3 on (t.id = S3.mi_movie_id AND S3.mi_movie_id = cc.movie_id AND S1.mc_movie_id = S3.mi_movie_id AND S3.mi_movie_id = S2.mk_movie_id ) 
...
JOIN (SELECT pi.person_id AS pi_person_id FROM person_info as pi join info_type as it3 on (it3.info = 'height' AND it3.id = pi.info_type_id)) S6 on (S4.ci_person_id = S6.pi_person_id AND S5.n_id = S6.pi_person_id ) 
join char_name AS chn on (chn.name = 'Queen' AND chn.id = S4.ci_person_role_id );