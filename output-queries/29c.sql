...
JOIN (SELECT mi.movie_id AS mi_movie_id FROM movie_info AS mi
 join info_type AS it on (mi.info IS NOT NULL AND (mi.info LIKE 'Japan:%200%' OR mi.info LIKE 'USA:%200%') AND it.info = 'release dates' AND it.id = mi.info_type_id ))S3 on (t.id = S3.mi_movie_id AND S3.mi_movie_id = cc.movie_id AND S1.mc_movie_id = S3.mi_movie_id AND S3.mi_movie_id = S2.mk_movie_id ) 
JOIN (SELECT ci.movie_id AS ci_movie_id,ci.person_id AS ci_person_id,n.id AS n_id FROM cast_info AS ci
 join role_type AS rt on (ci.note IN ('(voice)', '(voice: Japanese version)', '(voice) (uncredited)', '(voice: English version)') AND rt.role_t ='actress' AND rt.id = ci.role_id ) join char_name AS chn on (chn.id = ci.person_role_id ) join name AS n on (n.gender ='f' AND n.name LIKE '%An%' AND n.id = ci.person_id ))S4 on (t.id = S4.ci_movie_id AND S4.ci_movie_id = cc.movie_id AND S1.mc_movie_id = S4.ci_movie_id AND S4.ci_movie_id = S2.mk_movie_id AND S3.mi_movie_id = S4.ci_movie_id ) 
...
JOIN (SELECT pi.person_id AS pi_person_id FROM person_info AS pi
 join info_type AS it3 on (it3.info = 'trivia' AND it3.id = pi.info_type_id ))S5 on (S4.ci_person_id = S5.pi_person_id AND S4.n_id = S5.pi_person_id );