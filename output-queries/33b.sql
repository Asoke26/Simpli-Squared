...
join title AS t2 on (t2.production_year = 2007 AND t2.id = ml.linked_movie_id ) 
...
JOIN (SELECT mi_idx2.movie_id AS mi_idx2_movie_id FROM movie_info_idx as mi_idx2 join info_type as it2 on (mi_idx2.info < '3.0' AND it2.info = 'rating' AND it2.id = mi_idx2.info_type_id)) S2 on (t2.id = S2.mi_idx2_movie_id AND ml.linked_movie_id = S2.mi_idx2_movie_id ) 
JOIN (SELECT mc1.movie_id AS mc1_movie_id FROM movie_companies as mc1 join company_name as cn1 on (cn1.country_code = '[nl]' AND cn1.id = mc1.company_id)) S3 on (t1.id = S3.mc1_movie_id AND ml.movie_id = S3.mc1_movie_id AND S1.mi_idx1_movie_id = S3.mc1_movie_id ) 
