...
JOIN (SELECT mc1.movie_id AS mc1_movie_id FROM movie_companies AS mc1
 join company_name AS cn1 on (cn1.country_code = '[nl]' AND cn1.id = mc1.company_id ))S4 on (t1.id = S4.mc1_movie_id AND ml.movie_id = S4.mc1_movie_id AND S1.mi_idx1_movie_id = S4.mc1_movie_id );