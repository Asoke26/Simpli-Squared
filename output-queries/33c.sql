...
JOIN (SELECT mi_idx2.movie_id AS mi_idx2_movie_id FROM movie_info_idx AS mi_idx2
 join info_type AS it2 on (mi_idx2.info < '3.5' AND it2.info = 'rating' AND it2.id = mi_idx2.info_type_id ))S2 on (t2.id = S2.mi_idx2_movie_id AND ml.linked_movie_id = S2.mi_idx2_movie_id ) 
