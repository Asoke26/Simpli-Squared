...
JOIN (SELECT ci.movie_id AS ci_movie_id,ci.person_id AS ci_person_id,n.id AS n_id FROM cast_info AS ci
 join name AS n on (n.name_pcode_cf BETWEEN 'A' AND 'F' AND (n.gender='m' OR (n.gender = 'f' AND n.name LIKE 'B%')) AND ci.person_id = n.id ))S1 on (t.id = S1.ci_movie_id AND S1.ci_movie_id = ml.linked_movie_id ) 
