...
JOIN (SELECT ci.person_id AS ci_person_id,ci.movie_id AS ci_movie_id,t.id AS t_id FROM cast_info AS ci
 join role_type AS rt on (rt.role_t ='writer' AND ci.role_id = rt.id ) join title AS t on (ci.movie_id = t.id ))S1 on (n1.id = S1.ci_person_id AND a1.person_id = S1.ci_person_id ) 
