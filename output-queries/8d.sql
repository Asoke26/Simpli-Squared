...
JOIN (SELECT ci.person_id AS ci_person_id,ci.movie_id AS ci_movie_id FROM cast_info as ci join role_type as rt on (rt.role_t ='costume designer' AND ci.role_id = rt.id)) S1 on (n1.id = S1.ci_person_id AND an1.person_id = S1.ci_person_id ) 
