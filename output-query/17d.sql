...
JOIN (SELECT ci.movie_id AS ci_movie_id FROM cast_info as ci join name as n on (n.name LIKE '%Bert%' AND n.id = ci.person_id)) S2 on (S2.ci_movie_id = t.id AND S2.ci_movie_id = mc.movie_id AND S2.ci_movie_id = S1.mk_movie_id );