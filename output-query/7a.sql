...
JOIN (SELECT an.person_id AS an_person_id,n.id AS n_id FROM aka_name as an join name as n on (an.name LIKE '%a%' AND n.name_pcode_cf BETWEEN 'A' AND 'F' AND (n.gender='m' OR (n.gender = 'f' AND n.name LIKE 'B%')) AND n.id = an.person_id)) S1 on (S1.an_person_id = ci.person_id AND ci.person_id = S1.n_id ) 
